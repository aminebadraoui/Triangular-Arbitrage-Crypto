...
const getPrice = async (baseToken, swapToken, protocol) => {
  ...
  const token0 = await pool_contract.token0();
  const token1 = await pool_contract.token1();
  ...
  const reserveToken0 = await token0_contract.balanceOf(pool);
  const reserveToken1 = await token1_contract.balanceOf(pool);
